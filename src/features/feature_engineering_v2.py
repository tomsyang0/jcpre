...
def create_features_v2(df: pd.DataFrame, is_training: bool = True) -> pd.DataFrame:
    """
    构建预测特征（无数据泄露版本）：
    只使用赛前可用的数据，移除所有比赛结果相关的特征
    ...
    result_related_cols = [
        ...
        # 比赛结果（注意：result_1x2是目标变量，训练时需要保留）
        "ou25_result",
        # 让球结果相关
        "hcp_goal_diff", "hcp_result",
        # 投注结果相关
        "bonus_score",
        # 这些可能是基于本场比赛结果的统计，需要小心
        "home_score_ratio", "away_score_ratio",
        "same_home_score_ratio", "same_away_score_ratio",
    ]
    
    # 如果是训练模式，保留目标变量result_1x2
    if is_training:
        # 从移除列表中排除result_1x2
        if "result_1x2" in result_related_cols:
            result_related_cols.remove("result_1x2")
    
    # 只移除存在的列
    cols_to_drop = [col for col in result_related_cols if col in df_features.columns]
    if cols_to_drop:
        df_features = df_features.drop(columns=cols_to_drop)
        logger.info(f"移除了 {len(cols_to_drop)} 个结果相关特征")
...
def split_features_target_v2(df: pd.DataFrame, target_col: str = "result_1x2") -> tuple[pd.DataFrame, pd.Series]:
    """
    分离特征和目标变量（无数据泄露版本）
    
    注意：这个函数假设df已经通过create_features_v2处理过，
